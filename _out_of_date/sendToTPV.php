<?php
    ...
    $trz=true;
    require_once('../_DATA/systemData.php');
    require_once('classes_support.php');
    require_once('class_soapV1.php');
    if($trz)dbg::seeDetails($_POST,'Input data');
    /*
    // Data for casll the webservice of shopping cart
    $SYSTEM=System::memory();
    $SYSTEM['voucher']=$_POST['voucher'];
    if($trz) dbg::seeDetails($SYSTEM,"System Data:");
    // Get the data from the webservice
    $wb='www.indiroom.es/WSSHOPPINGCART/';
    $webservice="https://{$wb}WSCART.php";
    $oqpc=new oqpsoapClient(['location'=>$webservice,
        'uri'=>$webservice]);
    $d=$oqpc->gotoPay($SYSTEM);
    $e=$oqpc->getLastErrors();
    $trz=htmlentities($oqpc->getTrz(),ENT_QUOTES,"UTF-8");
    $z=[ 'data'=>$d, 'errorSOAP'=>$e, 'action'=>$action,
        'arguments'=>$arguments,'trace'=>$trz];
    if($trz) dbg::seeDetails($z,"WS answer"); //
    //
    */
    $clpayment=new TPVManager;
    $r=$clpayment->procesoNotificacionSIS($d,$trz);
    exit($r[0]);
...
class TPVManager {
	...
	public function procesoNotificacionSIS(& $d,$parar=false) {
		...
		if( isset($z['toPayment']) ) {
			...
			$data['amount'] *= 100;
			list($form,$pars)=$this->PrepareFormForCaixa($data,$parar);
		}
		else $error[]="Lacks the to pay section";
		return [$form,$pars,$error];
	}
	function PrepareFormForCaixa($data,$parar=false) {
		...
		foreach(self::translator() as $key=>$field)
			$p[$field]=$data[$key];
		require_once("apiRedsysPHP7.php");
		$miobj= new RedsysAPI;
	...
	private static function TPVData_design() {
		$utpv="https://www.indiroom.es/SIMULTPV";
        $u="https://www.indiroom.es/TPV";
		$d=[
			'OQP'=>[
				'callto'=>$utpv.'/makePayment.php',
				'prod'=>'0',
				'pwd'=>'sq7HjrUOBfKmC576ILgskD5srU870gJ7',
			],
			'Ds_Signature'=>'', #To be calculated with algorithm SHA-256
